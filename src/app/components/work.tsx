...
export default function Work({ title, tags, imagePath, alt, iconType, url, children }: Props) {
    const tagsEl = tags.map((tag, index) => {
        return <span key={index} className="inline-flex items-center rounded-md bg-gray-50 px-2 py-1 mr-1 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10">{tag}</span >
    })
    return (
        <li>
            <div className="flex items-center gap-x-6">
                ...
                {iconType === "Github" && <svg className="w-32 h-32 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                ...
                {iconType === "twitter" && <svg className="w-32 h-32 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
